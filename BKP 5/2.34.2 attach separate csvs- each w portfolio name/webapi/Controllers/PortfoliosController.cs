...
namespace webapi.Controllers
{
    ...
    public class PortfoliosController : ControllerBase
    {
        ...
        // (1) create csv (that contains just the names of the portfolios)
        // and (2) SEND EMAIL
        ...
        public IActionResult PostPortfolios([FromBody] portfoliosNumbers model)
        {
            ...
            // Attach each portfolio as a separate CSV file
            foreach (var portfolio in selectedPortfolioNames)
            {
                // Get the CSV content for the portfolio
                string csvContent = CsvService.GetCsvContent(new List<string> { portfolio });
